...
var HasChildViews = {
    ...
    replaceChildViewWith: function(identifier, NewChildView) {
        if (!this.hasChildViews()) {
            return;
        }

        var viewRelationship = this.childViews[identifier];
        var currentView = viewRelationship.childView;

        if (notInDOM(currentView)) {
            return;
        }
        ...
        this.createChildView(identifier, NewChildView)
            .andReplace(currentView.el);
    },
...
function notInDOM(view) {
    if (!view || Environment.isServer()) {
        return true;
    }

    if (view.enterDOM) {
        return !view.isAttached || !view.isInDOM;
    }

    return !document.documentElement.contains(view.el);
}
